...
extern "C" wchar_t DLLEXPORT *GetActivationKey()
{
/*	SYSTEMTIME time;
	GetSystemTime(&time);
	char lpszTime[256];
...
//	string md5_2;
	string actCode;
	wchar_t* w_ret = (wchar_t*)GlobalAlloc(GMEM_FIXED, 1000);
//	char lpszSN[1024];
...
extern "C" wchar_t _declspec(dllexport)  *GetCommonAppdataFolder(){
   wchar_t* szPath = (wchar_t*)GlobalAlloc(GMEM_FIXED, 1000);
...
extern "C" void _declspec(dllexport)  FreeMemory(wchar_t* p){
	GlobalFree(p);
	p = NULL;
}

extern "C" void _declspec(dllexport)  SetClearTypeOn()
{
//	bool retValue;
	BOOL bSmoothing;  
	...
	{  
		BOOL succeed = ::SystemParametersInfo(   SPI_SETFONTSMOOTHING,   TRUE,   NULL,   SPIF_UPDATEINIFILE | SPIF_SENDCHANGE); 
		if (!succeed){
			DWORD error = GetLastError();
		}
	}
	BOOL succeed = ::SystemParametersInfo(SPI_SETFONTSMOOTHINGTYPE, 0, (PVOID)FE_FONTSMOOTHINGCLEARTYPE, SPIF_UPDATEINIFILE | SPIF_SENDCHANGE);
	if (!succeed){
		DWORD error = GetLastError();
		error = error;
	}
...
/* boolean setClearTypeOff (); */
extern "C" void _declspec(dllexport) SetClearTypeOff()
{
//	bool retValue;
	::SystemParametersInfo(SPI_SETFONTSMOOTHINGTYPE, 0, (PVOID)FE_FONTSMOOTHINGSTANDARD, SPIF_UPDATEINIFILE | SPIF_SENDCHANGE);
//	retValue = true;
//	return retValue;
	return;
}
...
/*
extern "C" bool _declspec(dllexport) SetClearTypeContrast(UINT value)
{
	bool retValue;
	SystemParametersInfo(SPI_SETFONTSMOOTHINGCONTRAST, value, 0, SPIF_UPDATEINIFILE | SPIF_SENDCHANGE); 
	retValue = true;
	return retValue;
}
*/
/* boolean isClearTypeOn (); */
extern "C" BOOL _declspec(dllexport) IsClearTypeOn()
{
	BOOL retValue;
	UINT  result = 0;
	SystemParametersInfo(SPI_GETFONTSMOOTHINGTYPE, 0, &result, 0); 
	if (result == FE_FONTSMOOTHINGCLEARTYPE){
		retValue = true;
		return retValue;
	}
	retValue = FALSE;
